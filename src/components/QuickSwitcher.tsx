...
export const QuickSwitcher: React.FC<QuickSwitcherProps> = ({ vaultPath, onSelectNotePath }) => {
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState('');
  const [results, setResults] = useState<string[]>([]);
  const [selectedIndex, setSelectedIndex] = useState(0);

  // Open on Ctrl+O / Cmd+O
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (!query.trim()) {
          const res = await invoke<string[]>('search_titles', { vaultPath, prefix: '' });
          setResults(res || []);
        } else {
          const res = await invoke<string[]>('search_titles', { vaultPath, prefix: query.trim() });
          setResults(res || []);
        }
        setSelectedIndex(0);
      } catch (err) {
        console.error('search_titles error', err);
        setResults([]);
      }
    })();
    ...
    >
      ...
      >
        <div style={{ padding: '12px', borderBottom: '1px solid var(--border-color, #333)' }}>
          <input
            autoFocus
            placeholder="Quick Switch Note (Ctrl+O)..."
            value={query}
          ...
          {results.length === 0 ? (
            ...
            <ul style={{ listStyle: 'none', padding: '4px 0', margin: 0 }}>
              {results.map((title, idx) => (
                <li
                  key={idx}
                  onClick={() => handleSelect(title.endsWith('.md') ? title : `${title}.md`)}
                  style={{
                    padding: '8px 16px',
                    cursor: 'pointer',
                    fontSize: '13px',
