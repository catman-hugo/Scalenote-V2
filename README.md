...
- **Source of truth:** plain `.md` files with YAML frontmatter. Everything that counts as content has a visible, ordinary location on disk — a user (or a local AI agent with filesystem access) should be able to open the vault folder in any file browser and see everything that exists, with no opaque blobs standing in for real content. Concretely:
  ...
  MyVault/
    ...
    .scalenote/
      ...
      device-pairing/               ← 3DS pairing keys, local-only, not vault content
  ```
  ...
  - `.scalenote/` is reserved strictly for regenerable caches and device-local state (the search index, 3DS pairing keys) — never for anything that would be lost if that folder were deleted.
- **Collaborative document model:** `yrs` (Rust port of the Yjs CRDT) backs every note's content in memory, and is what gets serialized to the plain `.md` file (and `.canvas.json` sidecar, where present) on save. This is what makes real-time multi-editor sync and cursor awareness possible — see below.
...
- Markdown source editing with live preview (Obsidian-style)
- Block-based rich editing mode as an alternative view of the same file (Notion/AppFlowy-style): headings, bullet/numbered/todo lists, toggle lists, quote, code block with syntax highlighting (see below for the exact look), callout, divider, table, image embed
...
This isn't a walk-back of "no accounts" — it's a clarification of what that always meant. "No accounts" means no third party ever verifies your identity or holds your data. It never meant you can't have a name.

- **Local username:** every install can optionally set a display name — self-declared, stored only on that device, never verified or stored by anyone else. This is what shows up to other people during P2P collaboration and in Created-by/Last-edited-by database properties.
- **Local password:** optional app-lock, not authentication — there's no server to authenticate against. If set, it just gates opening the app on that device (useful on a shared family computer), and is stored (hashed) locally only.
- **Friend nicknames (private, local-only overrides):** every peer has a permanent identity underneath their changeable display name — the same cryptographic key already used for P2P pairing and trust. You can set a local nickname for any peer you've connected with, tied to *their key*, not their current display name — so if they rename themselves later, your nickname for them still follows the right person. This override is visible only on your device: if your friend Sam sets his own username to "Seal," you can privately label him "Sam" in your view, everyone else still sees "Seal," and Sam still sees himself as "Seal" — your nickname for him never gets broadcast to him or anyone else.
...
Real-time P2P collaboration above is for working with *other people*. This is the separate question of keeping *your own* laptop, desktop, and phone in sync with each other — three ways to do it, in increasing order of how much infrastructure they need:

- **Nothing extra:** your devices sync directly with each other exactly like the P2P collaboration above, whenever two of them happen to be online on the same network at the same time. Works, but both devices need to be reachable simultaneously.
- **Home server (recommended if you have an always-on machine):** designate one of your own paired devices as your home server — a machine that's usually on (Ron's example: a homelab server that's up whenever he's awake). This isn't a different sync system: it's the same `yrs` CRDT engine and QUIC connection already described above, just used in a hub pattern instead of live person-to-person editing. Configure it in Settings with a custom IP address and port (the same manual-entry mechanism as connecting to a peer outside your discovery range, just given a dedicated, persistent slot so it reconnects automatically rather than being a one-off). Each of your devices independently syncs with the home server whenever it's reachable; the CRDT merges correctly regardless of which devices were online when, so editing on your laptop while the server's down and having it catch up later works cleanly. A headless "server mode" build (no GUI, just the sync engine, meant to run unattended) is how this actually gets deployed on a homelab box — a Dockerfile, docker-compose.yml, and setup guide (including Proxmox-specific guidance: LXC-with-nesting vs. a small VM) already exist in `server/` in this repo, describing the target shape for that build. It's a deployment contract for a future binary, not a working server yet — see `server/SERVER.md` for the honest status.
  - **A home server can host more than one person's account,** cleanly isolated from each other — Ron's example: his girlfriend wants her own PC and laptop synced through his server too, without him ever seeing her credentials or her having to trust him with an account on some third system. Nobody hands over a username/password to the server owner at all — access works the same way Jellyseerr handles new user requests: someone points their client at the server's IP and port and sends a request to use it; the server owner's admin device (whichever device originally set the server up) gets a pending-request notification with green Accept / red Decline, exactly like an incoming call. Once accepted, that person's identity gets its own isolated space on the server — their content never mixes with anyone else's. An **allowlist** lets the owner pre-approve specific people (a partner, a close friend) so they're never prompted again, and a **blocklist** silently auto-declines future requests from a specific identity without even surfacing a prompt — the actual mitigation for someone spamming fake requests: it costs the owner nothing to decline, and a repeat offender just gets blocked outright.
- **Third-party cloud folder sync (Nextcloud, MEGAsync, Google Drive, Dropbox, or similar), for anyone without a home server:** since a vault is just plain files, pointing your vault folder at a folder one of these tools already syncs works today with no integration work on ScaleNote's part at all — no APIs, no accounts added to the app itself. **Important limitation, stated plainly rather than glossed over:** this only works as well as the *third-party tool's* conflict handling, because a generic file-sync tool has no idea what a CRDT is. If you edit the same note offline on two devices and the sync tool reconciles them later, you get whatever that tool does with a real conflict — usually a last-write-wins overwrite or a "conflicted copy" duplicate that needs manual merging, not the clean automatic merge the P2P/home-server options give you. This mode is a reasonable, genuinely free fallback, but it isn't equivalent to the other two — say so clearly in the app, not just in this document.
...
- Customizable sidebar: drag to reorder which sections appear (the main Notebook/Folder tree, Recents, Favorites, Tags, Upcoming, Shared, and any pinned database views), with an "Add section" picker to bring in ones that aren't currently shown, and a "Done" button to exit customize mode. Favorites (star/pin any note for quick access) and Upcoming (a local aggregation of notes with reminders set) are both new — not previously in this spec, worth having on their own merits, not just as customization filler. "Shared" here means notes with an active or past P2P collaboration session, not Notion's cloud-sharing concept.
...
## AI assistant (optional, the one feature that needs internet)

Everything else in ScaleNote works fully offline by design. This is the deliberate, narrow exception — browsing model catalogs, calling Ollama Cloud, and downloading model weights all require internet access. It's opt-in: the feature sits inert and makes no network calls at all until the user opens the AI tab and interacts with it. See CLAUDE.md for how this is scoped so it doesn't compromise the offline-first guarantee everywhere else.

Goal: someone with zero AI background should be able to open the AI tab and end up with a model that actually works well on their machine, without knowing what a GGUF or a quantization is going in.

- **Status banner:** shows whether Ollama is installed and running locally, and whether an Ollama Cloud API key is configured.
- **Guided setup for both, if missing:** if Ollama isn't installed, step-by-step instructions with a direct download link for the detected OS, and a "check again" button rather than requiring an app restart. If no Ollama Cloud API key is set, step-by-step instructions with real hyperlinks to where to generate one, plus a field to paste it in.
- **Hardware detection:** CPU, total system RAM, and GPU/VRAM, detected automatically the first time the tab is opened, with a manual "rescan hardware" button for after a hardware change.
- **Model catalog, five tabs:** Recommended / Browse Ollama / Browse HuggingFace (GGUF only) / Ollama Cloud / Installed.
- **Recommended is computed, not static:** actually estimate each model's VRAM/RAM requirement at its available quantization levels and score it against the detected hardware, rather than showing fixed badges that don't reflect the specific machine. A rough sense of the tiers to work from: no dedicated GPU → 1–4B models are the realistic range; ~8GB VRAM → 7–8B quantized; ~12GB VRAM → 12–14B quantized; 24GB+ VRAM → 27–32B becomes practical; 70B-class needs roughly 40GB even at 4-bit quantization. Treat these as a starting heuristic to refine, not a hardcoded lookup table — model efficiency varies by architecture and this shifts over time.
- **Plain-language warnings on every listed model, not just the recommended ones:**
  - Small models get an honest caveat (e.g. "Smaller and faster, but more likely to get complex questions wrong or make things up") rather than silence.
  - Models known to not support images get a clear "text only — can't look at images you share" label, not a silent gap someone discovers by trying.
  - Anything that would run CPU-only and be genuinely slow on the detected hardware is labeled as such before download, not after a frustrating wait.
- **HuggingFace GGUF browsing validates availability before offering a download** — check that a repo actually has a usable GGUF quantization before presenting it as a one-click option. A real, documented failure mode in Odysseus (the AI-workspace project this feature was modeled on) was letting users pick a model with no available GGUF and hitting a dead end after the fact; check first instead.
- **Study Assistants (RAG):** upload notes/textbooks/slides and get a version of any installed model that answers using that material — genuinely useful, worth keeping from Previous ScaleNote, which had this feature already.
- **Chat with memory:** chat with any configured model — local Ollama or Ollama Cloud — and let it build up durable memory of things worth remembering across conversations (your preferences, recurring context, decisions you've mentioned), modeled on Odysseus's memory system but adapted to fit here rather than copied wholesale.
  - **Extraction runs in the background, after a conversation, not live mid-chat** — the model reviews what was actually said and proposes candidate facts, rather than trying to extract in real time while you're still typing.
  - **A regex-based fallback catches obvious statements** ("my name is...", clear preference statements) when no model is available to do smarter extraction, or when the smarter extraction fails — don't let memory silently stop working just because the configured model is temporarily unreachable.
  - **Extracted facts are stored as real notes in the vault** — a dedicated folder, one small note per fact, with frontmatter for type (preference/fact/decision), date, and confidence — not a hidden database blob. This means memory is visible, editable, and deletable exactly like any other note, and it rides along with whatever sync (P2P, home server, cloud-folder fallback) is already in use, for free.
  - **A periodic consolidation pass merges duplicates and prunes low-value entries** — a real, documented failure mode in the reference system this was modeled on is unbounded memory growth over time ("memory bloat") if nothing ever cleans it up; build the cleanup in from the start rather than discovering the problem later.
  - **Retrieval is relevance-scoped, not a full dump** — only memories that are actually semantically relevant to the current conversation get pulled in, found via a local embedding search (see CLAUDE.md), not the entire memory store injected into every prompt.
  - **Memory is for atomic facts and preferences, not whole documents.** A real, reported problem with the reference system was people trying to use memory extraction to retain multi-page documents and getting them "shredded into individual facts" — the wrong shape entirely. That need is already covered by Study Assistants (RAG) above; keep the two clearly separate.
  - **Treat memory as content that steers future AI behavior, because it is** — every extracted memory gets fed back into future prompts, so a bad or malicious entry keeps affecting responses until it's removed. Never store passwords, API keys, tokens, or any other secret in memory — it isn't a secrets vault, and it's included in anything that reads it back later.
...
- **Shared sync core:** use `ctru-rs` (the community Rust toolchain for 3DS homebrew) so `yrs` can run on the 3DS too. Given the narrowed scope, the 3DS only ever needs to sync the canvas-layer CRDT structure and page titles/list — never the full document schema (blocks, text) — which meaningfully reduces what needs to be ported and transmitted.
...
- No central server of any kind, including for collaboration — peer-to-peer only, invite-code based
- No plugin system / third-party extension marketplace
- No AI features (no local LLM integration inside the app itself)
- No mobile app in this build (Android is a planned future target — see Build targets)
- No collaboration on the canvas/drawing mode in this build — real-time sync applies to text/block notes only for now; simultaneous multi-cursor drawing is a larger CRDT problem (ink strokes as CRDT elements) and is out of scope for this pass
